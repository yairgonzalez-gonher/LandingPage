...
export default function TikentaPricing({
  ...
  return (
    <section id="planes" className="py-20 md:py-28 bg-tk-bg">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="max-w-3xl mb-8">
          <p className="text-xs font-bold uppercase tracking-[0.16em] text-tk-brand mb-3">Planes</p>
          <h2 className="text-3xl md:text-5xl text-tk-text">El volumen que corresponde a tu operación.</h2>
          ...
          {cycles.map((item) => (
            <button
              ...
              className={`px-4 py-2 rounded-full text-sm font-bold border transition-colors ${
                cycle === item.id
                  ? 'bg-tk-dark text-white border-tk-dark'
                  : 'bg-white text-tk-text border-gray-200 hover:border-tk-brand'
              }`}
            >
              {item.label}
            </button>
        ...
        <div className="mb-4 p-4 md:p-5 rounded-2xl border border-tk-line bg-gradient-to-r from-tk-tint2 to-white flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <p className="font-semibold text-tk-text">Agregar Facturación a Clientes</p>
          ...
          <label className="inline-flex items-center gap-3 cursor-pointer self-start md:self-center">
            <span className="text-xs font-extrabold text-tk-text">{addonOn ? `Incluido · ${tier.name}` : 'No incluido'}</span>
            <span className="relative w-12 h-7">
              ...
              />
              <span className={`absolute inset-0 rounded-full transition-colors ${addonOn ? 'bg-tk-brand' : 'bg-gray-300'}`} />
        ...
        {addonOn && (
          <p className="text-xs font-extrabold text-tk-brand mb-4">
            {addon.amount === null
        ...
        {addonOn && (
          <div className="mb-6 p-4 md:p-5 rounded-2xl border border-tk-line bg-white">
            <p className="font-semibold text-tk-text mb-1">Selecciona el nivel de Facturación a Clientes</p>
              ...
              {Object.entries(addonPricing).map(([id, item]) => (
                <label
                  ...
                  className={`block rounded-xl border p-4 cursor-pointer transition-colors ${
                    addonTier === id ? 'border-tk-brand bg-tk-tint' : 'border-gray-200 hover:border-tk-line'
                  }`}
                >
                  <input
                    type="radio"
                  ...
                  />
                  <span className="block text-xs font-extrabold text-tk-text">{item.name}</span>
            ...
            return (
              <article
                ...
                className={`relative flex flex-col rounded-2xl border bg-white p-6 overflow-hidden ${
                  plan.popular ? 'border-tk-brand shadow-[0_16px_40px_rgba(79,46,211,0.14)]' : 'border-gray-200'
                }`}
              >
                {plan.popular && <span className="absolute top-0 left-0 right-0 h-1 bg-tk-brand" />}
                {plan.popular && (
                  <span className="self-start mb-2 text-[10px] font-extrabold uppercase tracking-wide bg-tk-tint text-tk-brand px-2 py-1 rounded-md">
                    Más popular
                  </span>
                )}
                <h3 className="text-xl font-semibold text-tk-text">{plan.name}</h3>
                <p className="text-sm text-gray-500 mt-1 min-h-[48px]">{plan.desc}</p>
                <p className="tikenta-num text-3xl font-bold text-tk-text mt-3">{priceLabel}</p>
                <p className="text-[11px] text-gray-500 mt-1">{base.period}</p>
                {showTotal && (
                  <p className="mt-2 text-[11px] leading-relaxed rounded-lg bg-tk-tint border border-tk-line text-tk-ink p-2">
                ...
                <ul className="mt-4 mb-6 space-y-2 text-sm text-gray-600 flex-1">
                  {plan.features.map((feature) => (
                    <li key={feature} className="flex gap-2">
                      <span className="text-tk-brand font-black">✓</span>
                      {feature}
                    </li>
                  ))}
                </ul>
                <button
                  ...
                  className={`w-full rounded-lg py-3 text-sm font-bold transition-colors ${
                    plan.popular
                      ? 'bg-tk-brand text-white hover:bg-tk-deep'
                      : 'border border-gray-200 bg-white text-tk-text hover:border-tk-brand hover:text-tk-brand'
                  }`}
                >
                  {ctaLabel}
                </button>
        ...
        <p className="mt-5 text-sm text-tk-brand rounded-xl border border-tk-line bg-tk-tint p-4">
