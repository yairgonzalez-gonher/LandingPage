...
export default function TikentaClose({ onDemoOpen }) {
  return (
    <>
      <section className="bg-carbon text-white py-20 md:py-24">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 grid lg:grid-cols-2 gap-10 items-center">
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.16em] text-steel-400 mb-3">Control y seguridad</p>
      ...
      <section id="faq" className="py-20 md:py-28 bg-white">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          <p className="text-xs font-bold uppercase tracking-[0.16em] text-steel mb-3">Preguntas frecuentes</p>
          <h2 className="text-3xl md:text-5xl text-carbon mb-8">Lo importante antes de contratar.</h2>
          {faqs.map((item) => (
            <details key={item.q} className="group border-b border-gray-200 py-4">
              <summary className="cursor-pointer font-bold text-carbon list-none flex justify-between gap-4 items-center">
                {item.q}
                <span className="text-steel text-xl leading-none group-open:hidden">+</span>
                <span className="text-steel text-xl leading-none hidden group-open:inline">−</span>
              </summary>
      ...
      <section className="pb-20 md:pb-24 bg-white">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="relative overflow-hidden rounded-2xl bg-carbon text-white p-8 md:p-12 flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div className="absolute -right-16 -top-20 h-56 w-56 rounded-full bg-steel/30 pointer-events-none" />
            ...
            <button
              type="button"
              onClick={onDemoOpen}
              className="relative inline-flex justify-center rounded-lg bg-white text-carbon font-bold px-5 py-3.5 hover:bg-gris-claro transition-colors shrink-0"
            >
              Solicitar demo
            </button>
          </div>
      ...
      <footer className="bg-[#111418] text-white/50 py-12">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 grid md:grid-cols-[2fr_1fr_1fr] gap-8">
          <div>
            <Logo wordmarkClassName="text-white" imgClassName="h-9 w-auto" />
            <p className="mt-3 text-sm max-w-md">
              Tikenta: captura, validación, facturación y análisis de tickets de gasolina y casetas para empresas en México.
            </p>
          </div>
          <div>
