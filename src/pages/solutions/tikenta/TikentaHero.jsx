...
function TypePill({ type }) {
  ...
  return (
    <span className={`inline-flex px-2 py-0.5 rounded-full text-[10px] font-extrabold ${gas ? 'bg-emerald-50 text-emerald-800' : 'bg-slate-100 text-slate-700'}`}>
      {type}
    </span>
  );
}
...
export default function TikentaHero({ onDemoOpen }) {
  return (
    <section id="inicio" className="relative overflow-hidden bg-carbon text-white">
      <div className="tikenta-dash-grid absolute inset-0 opacity-30 pointer-events-none" />
      <div className="absolute -right-24 top-20 h-72 w-72 rounded-full bg-steel/20 blur-3xl pointer-events-none" />
      ...
      <div className="relative max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-16 md:py-24 grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
        <div>
          <p className="inline-flex items-center gap-2 text-[11px] font-extrabold uppercase tracking-[0.14em] text-steel-400">
            <span className="h-1.5 w-1.5 bg-steel rounded-[2px]" />
            Producto de GONSoftLab
          </p>
          <h1 className="mt-5 text-[42px] sm:text-5xl lg:text-6xl font-bold leading-[1.05]">
            Controla tickets de gasolina y casetas, no una carpeta de <span className="tikenta-serif text-steel-400">comprobantes</span>.
          </h1>
          ...
          <div className="mt-8 flex flex-col sm:flex-row gap-3">
            <a href="#planes" className="inline-flex justify-center items-center rounded-lg bg-steel px-5 py-3.5 font-bold hover:bg-steel-700 transition-colors">
              Ver planes
            </a>
            <button
              type="button"
              onClick={onDemoOpen}
              className="inline-flex justify-center items-center rounded-lg bg-white px-5 py-3.5 font-bold text-carbon hover:bg-gris-claro transition-colors"
            >
          ...
          <ul className="mt-8 flex flex-wrap gap-x-5 gap-y-2 text-sm text-white/55">
            {['Tickets de combustible', 'Tickets de casetas', 'OCR y validación', 'Reportes de gastos'].map((item) => (
              <li key={item} className="flex items-center gap-2">
                <span className="h-[18px] w-[18px] rounded-[5px] bg-steel/20 text-steel-400 grid place-items-center text-[10px] font-black">✓</span>
                {item}
              </li>
            ))}
          </ul>
        ...
        <div className="relative">
          <div className="absolute -left-6 -top-8 hidden xl:block w-44 -rotate-[8deg] rounded-md border border-white/10 bg-[#fffef8] text-carbon p-3 shadow-2xl">
          ...
          <div
            className="relative rounded-2xl overflow-hidden bg-white text-carbon shadow-[0_24px_60px_rgba(0,0,0,0.35)] border border-white/10"
          ...
          >
            ...
            <div className="grid grid-cols-[118px_1fr] sm:grid-cols-[132px_1fr] min-h-[360px] bg-[#f4f6f8]">
              <aside className="bg-carbon text-white/55 p-3.5 text-[11px]">
                <div className="flex items-center gap-2 text-white font-bold pb-3 mb-2 border-b border-white/10">
                  <span className="h-7 w-7 rounded-lg bg-steel grid place-items-center text-white text-xs">T</span>
                  TIKENTA
                </div>
                {['Inicio', 'Tickets', 'Facturas', 'Reportes', 'Usuarios'].map((item) => (
                  <div
                    key={item}
                    className={`mt-1 rounded-lg px-2 py-1.5 ${item === 'Tickets' ? 'bg-steel text-white font-bold' : ''}`}
                  >
                    {item}
                  </div>
                ))}
